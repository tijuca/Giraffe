...
class CZarafaApp : public CWinApp
{
public:
// Overrides
	// ClassWizard generated virtual function overrides
	//{{AFX_VIRTUAL(CZarafaApp)
	public:
...
BEGIN_MESSAGE_MAP(CZarafaApp, CWinApp)
	//{{AFX_MSG_MAP(CZarafaApp)
...
BOOL CZarafaApp::InitInstance()
{
	...
	if(RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("Software\\Zarafa\\Client"), 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
		...
		if(RegQueryValueEx(hKey, _T("InstallDir"), NULL, NULL, szDir, &cbDir) == ERROR_SUCCESS)
			g_strZarafaDirectory = (LPTSTR)szDir;
	...
	g_strCommonFilesZarafa = (LPTSTR)szDir;
	g_strCommonFilesZarafa += _T("\\") + g_strManufacturer;

	strPath = g_strCommonFilesZarafa + _T("\\locale");
	...
	if(SUCCEEDED(SHGetFolderPath(NULL, CSIDL_LOCAL_APPDATA, NULL, SHGFP_TYPE_CURRENT, (LPTSTR)szDir))) {
		g_strUserLocalAppDataZarafa = (LPTSTR)szDir;
		g_strUserLocalAppDataZarafa += _T("\\");
		g_strUserLocalAppDataZarafa += g_strManufacturer.c_str();
	}
...
HRESULT InitializeProvider(LPPROVIDERADMIN lpAdminProvider, IProfSect *lpProfSect, sGlobalProfileProps sProfileProps, ULONG *lpcStoreID, LPENTRYID *lppStoreID)
{
	...
	if(ulResourceType == MAPI_STORE_PROVIDER)
	{
		...
		sPropVals[cPropValue++].Value.lpszA = const_cast<char *>(ZARAFA_DLL_NAME);
		...
		sPropVals[cPropValue].ulPropTag = PR_DISPLAY_NAME_A;
		sPropVals[cPropValue++].Value.lpszA = const_cast<char *>("Zarafa Addressbook");
				...
				if(bShowAllSettingsPages)
				{
					...
					if(cDeligateStores >= sizeof(MAPIUID))
					{
						...
						for (d = 0; d < cDeligateStores / sizeof(MAPIUID); ++d) {
							...
							if (ptrProfSectProps[1].Value.bin.cb != sizeof(MAPIUID) || memcmp(ptrProfSectProps[1].Value.bin.lpb, &ZARAFA_STORE_DELEGATE_GUID, sizeof(MAPIUID)) != 0)
								continue;
...
#ifdef WIN32
// Add zarafa profile in MAPISVC.ini
...
// Remove zarafa profile from MAPISVC.ini
