 ...
 *
 * Since Zarafa client is multi threaded, we must collect info in a struct and map per pthread (connection).
 *
...
{
	return ssl_verify_callback_zarafa_control(ok, store, TRUE);
}
...
#ifdef WIN32
		if(ssl_verify_callback_zarafa_control(ok, store, FALSE) == 1) {
...
HRESULT LoadCertificatesFromRegistry()
{
	...
	if (RegOpenKeyExA(HKEY_CURRENT_USER, "Software\\Zarafa\\Client\\SSL", 0, KEY_READ, &keySSL) != ERROR_SUCCESS)
