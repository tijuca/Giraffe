...
, m_ulStreamBatchSize(256)
{
	 ...
	 * to enable SyncLog set 1 (REG_DWORD) on to HKEY_CURRENT_USER\Software\Zarafa\Client\SyncLog
	 * to set SyncLogLevel set 1-6 (REG_DWORD) on HKEY_CURRENT_USER\Software\Zarafa\Client\SyncLogLevel
	 *
	 ...
	 **/
	if (RegOpenKeyEx(HKEY_CURRENT_USER, _T("Software\\Zarafa\\Client"), 0, KEY_READ, &hKey) == ERROR_SUCCESS){
		ulSize = sizeof(ULONG);
