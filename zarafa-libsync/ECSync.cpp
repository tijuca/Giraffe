...
void ECSync::init(){
	...
	std::wstring strManufacturer = _T("Zarafa");
	DWORD cbData = 0;
	DWORD dwData = 0;
	...
	if(RegOpenKeyEx(HKEY_CURRENT_USER, _T("Software\\Zarafa\\Extension"), 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
		cbData = sizeof(DWORD);
...
HRESULT ECSync::Logon(){
...

	{
		...
		// an old zarafa client to a new one.
...
HRESULT ECSync::HrCreateRemindersFolder(){
	...
	//named property reminder set, doesn't need getidsfromnames in zarafa
	...
	//named property is recurring, doesn't need getidsfromnames in zarafa
